...
<template>
    <div class="flex flex-col w-full h-screen bg-gray-50 pt-8 items-center md:mt-14 mt-14 lg:mt-12">
        <div class="bg-gray-400 dark:bg-gray-700 p-5 pt-10 w-full">
            <h2 class="text-3xl font-bold text-white text-center">All Orders</h2>
        </div>
        ...
        <div v-else class="flex flex-col w-full h-screen bg-gray-50 pt-8 items-center mt-2">
            <div v-if="orderStore.orders && orderStore.orders.length > 0" class="flex flex-col w-full h-screen bg-gray-50 pt-12 items-center">
                <ul class="w-full max-w-4xl mx-auto px-4 md:px-20">
                    <li v-for="order in orderStore.orders" :key="order.id" class="mb-4 p-4 md:p-10 bg-white rounded-lg shadow flex justify-between items-center">
                        ...
                        <RouterLink :to="{ name: 'manager.orders.detail', params: { id: order.id } }" class="ml-4 md:ml-20 px-4 py-2 md:py-4 text-white bg-gray-700 hover:bg-gray-900 focus:outline-none font-medium rounded-xl text-sm md:text-base text-center"> View details </RouterLink>
                    </li>
                </ul>
            </div>
