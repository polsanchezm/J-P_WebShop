...
const updateVariant = handleSubmit((values) => {
    ...
    const productData = { color: values.color, stock: values.stock, size: values.size, id: editVariantId.value, productId: productId };
...
<template>
    <div v-if="productServ.oneProductDetail.value" class="max-w-md mx-auto bg-white shadow-md rounded px-4 py-6">
        ...
        <p class="text-gray-700"><span class="font-semibold">Name:</span> {{ productServ.oneProductDetail.value!.name }}
        </p>
        <p class="text-gray-700"><span class="font-semibold">Description:</span> {{
            productServ.oneProductDetail.value!.description }}</p>
        <p class="text-gray-700"><span class="font-semibold">Price:</span> {{ productServ.oneProductDetail.value!.price
            }}</p>
        <p class="text-gray-700"><span class="font-semibold">Category ID:</span> {{
            productServ.oneProductDetail.value!.categoryId }}</p>
        ...
        <RouterLink
            ...
            :to="{ name: 'manager.products.edit', params: { id: productServ.oneProductDetail.value!.id } }">Edit
        </RouterLink>

        <button @click="managerProductServ.deleteProduct(productServ.oneProductDetail.value!.id)"
            class="inline-block mt-4 text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 lg:py-2.5 mr-2 focus:outline-none">Delete</button>
                ...
                <div class="p-6 space-y-6 sm:p-8">
                    ...
                    <form class="max-w-md mx-auto" @submit.prevent="createVariant">
                        <div class="relative z-0 w-full mb-5 group">
                            ...
                            <label for="size"
                                class="peer-focus:font-medium absolute text-sm text-metal-600 peer-focus:text-metal-600 dark:text-ecru-200 peer-focus:dark:text-ecru-50 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Variant
                                size</label>
                        </div>
                        <div class="relative z-0 w-full mb-5 group">
                        ...
                        <button type="submit"
                            class="w-full text-gray-50 bg-gray-700 hover:bg-gray-900 dark:text-ecru-50 dark:bg-ecru-950 dark:hover:bg-ecru-700 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center">Add
                            Variant</button>
                    </form>
                </div>
            </div>
        ...
        <ul v-if="productServ.productVariants">
            <li v-for="(productVariant, index) in productServ.productVariants.value" :key="index">
                ...
                <button @click="managerProductServ.deleteVariant(productVariant!.id)"
                    class="inline-block mt-4 text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 lg:py-2.5 mr-2 focus:outline-none">Delete
                    Variant</button>
                    ...
                    <div class="bg-gray-50 dark:bg-corduroy-900 rounded-lg shadow">
                        <div class="p-6 space-y-6 sm:p-8">
                            ...
                            <form class="max-w-md mx-auto" @submit.prevent="updateVariant">
                                <div class="relative z-0 w-full mb-5 group">
                                    <Field name="size" id="size" as="select" v-model="productVariant!.size"
                                        class="block py-2.5 px-0 w-full text-sm text-metal-600 border-metal-600 focus:border-metal-950 dark:text-ecru-50 dark:border-ecru-300 dark:focus:border-ecru-50 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 peer"
                                        placeholder=" " required>
                                        <option :value="'XS'">XS</option>
                                        <option :value="'S'">S</option>
