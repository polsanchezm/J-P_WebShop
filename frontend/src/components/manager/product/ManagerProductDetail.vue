...
<template>
    <div class="bg-gray-50 mt-20 pt-8 min-h-screen">
            ...
            <div class="container mx-auto px-6 pb-28">
                ...
                <div v-else>
                                ...
                                <div class="mt-3">
                                    ...
                                    <div class="flex items-center space-x-3">
                                        <div v-for="color in uniqueColors" :key="color" class="relative">
                                            <input type="radio" :id="'color-' + color" v-model="selectedColor"
                                                :value="color" class="hidden cursor-pointer h-7 w-7" />
                                            <label :for="'color-' + color"
                                                :class="['inline-block h-7 w-7 rounded-full cursor-pointer', selectedColor === color ? 'ring-2 ring-gray-700 ring-offset-2 ring-offset-white' : '']"
                    ...
                    <div class="flex flex-wrap gap-4 mt-8 justify-center">
                        <RouterLink
                            class="text-center text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-xl text-sm px-4 py-4 focus:outline-none"
                            :to="{ name: 'manager.products.edit', params: { id: productStore.productDetail!.id } }">Edit
                            Product</RouterLink>
                        <button @click="productStore.deleteProduct(productStore.productDetail!.id)"
                            class="text-center text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:ring-red-300 font-medium rounded-xl text-sm px-4 py-4 focus:outline-none">Delete
                            Product</button>
                        <button @click="toggleAddVariant"
                            class="text-center text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-xl text-sm px-4 py-4 focus:outline-none">
                    ...
                    <div v-if="addVariantAreVisible" class="mt-8 px-4 py-6">
                        <div class="bg-gray-50 dark:bg-corduroy-900 rounded-xl shadow">
                            <div class="p-6 space-y-6 sm:p-8">
                                <h1
                                    class="text-center text-xl font-bold leading-tight tracking-tight text-corduroy-900 dark:text-ecru-50 md:text-2xl">
                                    Add Variant</h1>
                                <form class="max-w-md mx-auto" @submit.prevent="handleCreate">
                                    <div class="relative z-0 w-full mb-5 group">
                                        ...
                                        <label for="size"
                                            class="peer-focus:font-medium absolute text-sm text-metal-600 peer-focus:text-metal-600 dark:text-ecru-200 peer-focus:dark:text-ecru-50 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Variant
                                            size</label>
                                    </div>
                                    <div class="relative z-0 w-full mb-5 group">
                                        <Field name="color" v-slot="{ field, meta }">
                                            <input type="color" id="color" v-bind="field"
                                                class="block w-full h-10 p-2 text-sm text-gray-900 bg-transparent rounded-md appearance-none focus:outline-none focus:border-transparent dark:text-ecru-50 dark:border-ecru-300 dark:focus:ring-ecru-50"
                                                required />
                                            <label for="color"
                                                class="absolute text-sm text-gray-600 duration-300 transform -translate-y-6 scale-75 top-3 origin-[0] left-2.5 peer-focus:left-2.5 peer-focus:text-blue-600 peer-focus:dark:text-ecru-50 dark:text-ecru-200 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-placeholder-shown:top-3 peer-focus:scale-75 peer-focus:-translate-y-6">
                                                Variant color
                                            </label>
                                            <ErrorMessage v-if="meta.touched && meta.dirty" name="color"
                                    ...
                                    <div class="relative z-0 w-full mb-5 group">
                                        <Field name="stock" v-slot="{ field, meta }">
                                            ...
                                            <label for="stock"
                                                class="peer-focus:font-medium absolute text-sm text-metal-600 peer-focus:text-metal-600 dark:text-ecru-200 peer-focus:dark:text-ecru-50 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Variant
                                                stock</label>
                                            <ErrorMessage v-if="meta.touched && meta.dirty" name="stock"
                                                class="text-red-500 text-xs mt-1" />
                                        </Field>
                                    </div>
                                    <button type="submit"
                                        class="w-full text-gray-50 bg-gray-700 hover:bg-gray-900 dark:text-ecru-50 dark:bg-ecru-950 dark:hover:bg-ecru-700 focus:outline-none font-medium rounded-xl text-sm px-5 py-2.5 text-center">Add
                                        Variant</button>
                                </form>
                            </div>
                        </div>
                    ...
                    <div v-if="productStore.productVariants.length > 0" class="mt-8">
                        ...
                        <ul class="flex flex-wrap gap-4 justify-center">
                            <li v-for="(productVariant, index) in productStore.productVariants" :key="index"
                                class="bg-gray-100 dark:bg-gray-800 rounded-xl p-4 w-96">
                                <p class="text-gray-700"><span class="font-semibold">ID:</span> {{ productVariant.id }}
                                </p>
                                <p class="text-gray-700"><span class="font-semibold">Size:</span> {{ productVariant.size
                                    }}</p>
                                <p class="text-gray-700"><span class="font-semibold">Color:</span> {{
                                    productVariant.color }}</p>
                                <p class="text-gray-700"><span class="font-semibold">Stock:</span> {{
                                    productVariant.stock }}</p>
                                <p class="text-gray-700"><span class="font-semibold">Product ID:</span> {{
                                    productVariant.productId }}</p>
                                <div class="flex items-center justify-center gap-4 mt-4">
                                    <button @click="toggleEditVariant(index, productVariant.id)"
                                        class="text-center text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-xl text-sm px-8 py-4 focus:outline-none">
                                        {{ editVariantAreVisible === index ? 'Cancel' : 'Edit' }}
                                    </button>
                                    <button @click="productStore.deleteVariant(productVariant)"
                                        class="text-center text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-xl text-sm px-8 py-4 focus:outline-none">Delete</button>
                                </div>

                                <div v-if="editVariantAreVisible === index" class="mt-8 px-4 py-6">
                                    <div
                                        class="bg-gray-50 dark:bg-corduroy-900 rounded-xl shadow w-full max-w-2xl mx-auto">
                                        <div class="p-6 space-y-6 sm:p-8">
                                            <h1
                                                class="text-center text-xl font-bold leading-tight tracking-tight text-corduroy-900 dark:text-ecru-50 md:text-2xl">
                                                Edit Variant</h1>
                                            <form class="max-w-lg mx-auto" @submit.prevent="handleUpdate">
                                                ...
                                                <button type="submit"
                                                    class="w-full text-gray-50 bg-gray-700 hover:bg-gray-900 dark:text-ecru-50 dark:bg-ecru-950 dark:hover:bg-ecru-700 focus:outline-none font-medium rounded-xl text-sm px-5 py-2.5 text-center">Edit
                                                    Variant</button>
                                            </form>
                                        </div>
                                    </div>
