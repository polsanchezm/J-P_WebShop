        ...
        <ul>
            <li v-for="(shipping, index) in shippingStore.shipping" :key="index">
                <div v-if="shipping!.userId === authStore.user!.id">
                    ...
                    <button class="inline-block mt-4 text-white bg-primary-700 hover:bg-primary-800 font-medium rounded-lg text-sm px-4 py-2 lg:py-2.5 mr-2 focus:outline-none" @click="shippingStore.deleteShipping(shipping.id)">Delete</button>
                </div>
            </li>
        </ul>
    </div>
    <div v-else class="max-w-md mx-auto mt-24 bg-white shadow-md rounded px-4 py-6">
        <RouterLink class="gradient-button inline-block mt-4 text-white bg-primary-700 hover:bg-primary-800 font-medium rounded-lg text-sm px-4 py-2 lg:py-2.5 mr-2 focus:outline-none" :to="{ name: 'shipping.create' }">Create details</RouterLink>
    </div>
</template>
