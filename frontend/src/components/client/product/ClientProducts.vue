    ...
    <div class="min-h-screen bg-gray-100 flex flex-col justify-center">
        <div class="relative m-3 flex mx-auto justify-center">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-7xl w-full">
                <div v-for="(product, index) in productStore.products" :key="index" class="relative bg-white shadow-md rounded-3xl p-2 cursor-pointer" style="min-height: 500px">
                    <RouterLink :to="{ name: 'products.detail', params: { id: product.id } }">
                        <div class="overflow-x-hidden rounded-2xl relative">
                            <img class="h-96 rounded-2xl w-full object-cover" :src="product.image" />
