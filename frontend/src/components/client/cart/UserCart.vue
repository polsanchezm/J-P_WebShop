    ...
    <div v-if="cartStore.cart" class="max-w-md mx-auto bg-white shadow-md rounded px-4 py-6 mt-24">
        ...
        <ul>
            <li v-for="(item, index) in cartStore.cart" :key="index">
                ...
                <button @click="cartStore.decrementQuantity(index)" class="inline-block mt-4 text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 lg:py-2.5 mr-2 focus:outline-none">-</button>
                <button @click="cartStore.incrementQuantity(index)" class="inline-block mt-4 text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 lg:py-2.5 mr-2 focus:outline-none">+</button>
            ...
            <button v-if="cartStore.cart.length > 0" @click="cartStore.initiatePayment(cartItems)" class="inline-block mt-4 text-white bg-blue-500 hover:bg-blue-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 focus:outline-none">Pay with Stripe</button>
        </ul>
