...
<template>
    <div v-if="cartStore.cart" class="max-w-md mx-auto bg-white shadow-md rounded px-4 py-6 mt-24">
        ...
        <ul>
            ...
            <button v-if="cartStore.cart.length > 0" @click="cartServ.initiateStripePayment(cartItems)" class="inline-block mt-4 text-white bg-blue-500 hover:bg-blue-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 focus:outline-none">Pay with Stripe</button>
        </ul>
