    ...
    <div v-if="orderStore.oneOrder" class="max-w-md mx-auto bg-white shadow-md rounded px-4 py-6 mt-24">
        ...
        <ul>
            <li v-for="(oneOrder, index) in orderStore.oneOrder" :key="index">
                ...
                <p class="text-gray-700"><span class="font-semibold">Variant ID:</span> {{
                    oneOrder!.productVariant.id }}</p>
                ...
                <p class="text-gray-700"><span class="font-semibold">Product ID:</span> {{
                    oneOrder!.productVariant.productId }}</p>
            </li>
        </ul>
        <button @click="orderStore.deleteOrder(orderId, true)"
