...
<template>
    <div v-if="userStore.user" class="max-w-md mx-auto bg-white shadow-md rounded px-4 py-6">
        <p class="text-lg font-semibold mb-2 text-gray-700">User Information</p>
        <div>
            <p class="text-gray-700"><span class="font-semibold">ID:</span> {{ userStore.user!.id }}</p>
            <p class="text-gray-700"><span class="font-semibold">Name:</span> {{ userStore.user!.name }}</p>
            <p class="text-gray-700"><span class="font-semibold">Surnames:</span> {{ userStore.user!.surnames }}</p>
            <p class="text-gray-700"><span class="font-semibold">Birthdate:</span> {{
                formatDate(userStore.user!.birthdate) }}</p>
            <p class="text-gray-700"><span class="font-semibold">Email:</span> {{ userStore.user!.email }}</p>
            <RouterLink
