...
<template>
    <div v-if="authStore.user" class="max-w-md mx-auto bg-white shadow-md rounded px-4 py-6">
        ...
        <div>
            ...
            <button v-if="authStore.user.role === 'client'" @click="authStore.deleteUser()" class="inline-block mt-4 text-white bg-primary-700 hover:bg-primary-800 font-medium rounded-lg text-sm px-4 py-2 lg:py-2.5 mr-2 focus:outline-none">Delete</button>
        </div>
    </div>
</template>
