    ...
    <div v-if="orderStore.orders" class="max-w-md mx-auto bg-white shadow-md rounded px-4 py-6">
        ...
        <ul>
            <li v-for="(order, index) in orderStore.orders" :key="index">
                ...
                <RouterLink class="inline-block mt-4 text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 lg:py-2.5 mr-2 focus:outline-none" :to="'/user/orders/detail/' + order.id">View details</RouterLink>
                <button @click="orderStore.deleteUserOrder(order!.id)" class="inline-block mt-4 text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 lg:py-2.5 mr-2 focus:outline-none">Cancel order</button>
            </li>
        </ul>
